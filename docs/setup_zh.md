...
中文源通过直连 HTTP 抓取获取数据。三个候选源经实测：

- **量子位（`qbitai.com`）**：✅ 可用。非中国 IP 可访问，文章页返回 200，标题/摘要/日期可提取。当前 MVP 唯一的中文数据源。
- **机器之心（`jiqizhixin.com`）**：❌ SSL 握手超时。站点对非中国 IP 做了访问限制，且 DeepSeek API 的 `enable_search` 参数实际不触发网络搜索。
- **知乎（`zhihu.com`）**：❌ 需要登录认证，无法自动化采集。

实施规则：

- 抓取结果在清洗和验证之前单独评估字段完整度。
- 仅当结果包含标题、来源、发布日期（或 URL 可推断日期）、URL 以及摘要/正文时才接受进入主数据集。
- 在源评估备注中记录被拒绝或不可达的源，以说明源选择的权衡。
